...
<form class="admin-panel admin-form-panel" method="post" enctype="multipart/form-data" action="<?= $isEdit ? url('/admin/cotizaciones/editar?id=' . (int) $quote['id']) : url('/admin/cotizaciones/crear') ?>" id="quote-form">
    <?= csrf_field() ?>
    ...
    <div class="admin-form-section"><div><span>05</span><h2>Adjuntos</h2><p>Incorpore documentos complementarios que viajarán en el mismo correo que la cotización.</p></div><div><label class="form-label" for="quote-attachments">PDF, Word o imágenes</label><input class="form-control" id="quote-attachments" type="file" name="attachments[]" multiple accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/jpeg,image/png,image/webp"><div class="form-text">Máximo 5 archivos por cotización, 8 MB por archivo y 15 MB en total. Formatos: PDF, DOC, DOCX, JPG, PNG y WEBP.</div><?php if ($isEdit && !empty($quote['attachments'])): ?><div class="quote-attachment-summary mt-3"><strong>Adjuntos actuales</strong><?php foreach ($quote['attachments'] as $attachment): ?><span><?= e($attachment['original_name']) ?> · <?= e(number_format(((int) $attachment['file_size']) / 1048576, 2, ',', '.')) ?> MB</span><?php endforeach; ?></div><?php endif; ?></div></div>
