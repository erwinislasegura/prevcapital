...
<form class="admin-panel admin-form-panel" method="post" action="<?= $isEdit ? url('/admin/users/edit?id=' . (int) $user['id']) : url('/admin/users/create') ?>">
    ...
    <div class="admin-form-actions"><a class="btn admin-secondary-btn" href="<?= url('/admin/users') ?>">Cancelar</a><button class="btn admin-primary-btn" type="submit"><?= $isEdit ? 'Guardar cambios' : 'Crear usuario' ?></button></div>
</form>
