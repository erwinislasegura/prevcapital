...
<form class="admin-panel admin-form-panel" method="post" action="<?= $isEdit ? url('/admin/roles/edit?id=' . (int) $role['id']) : url('/admin/roles/create') ?>">
    ...
    <div class="admin-form-actions"><a class="btn admin-secondary-btn" href="<?= url('/admin/roles') ?>">Cancelar</a><button class="btn admin-primary-btn" type="submit"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12 4 4L19 6"></path></svg><?= $isEdit ? 'Guardar permisos' : 'Crear rol' ?></button></div>
</form>
